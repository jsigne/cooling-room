...
public interface CoolingRepository extends JpaRepository<Cooling, Integer> {
    ...
    @Query("SELECT new com.example.cold_room.model.RoomConsumption(c.idRoom, c.consumption, c.messageDate) FROM Cooling c WHERE c.idRoom = :idRoom")
    List<RoomConsumption> getConsumptionsByIdRoom(Integer idRoom);

    @Query("""
            SELECT new com.example.cold_room.model.Room(cooling.idRoom, cooling.isCooling, cooling.temperature, cooling.consumption, cooling.messageDate, alert.isAlert, alert.isWarning)
